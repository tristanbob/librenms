...
/**
 * WirelessGraphDefinitionResolver.php
 *
...
class WirelessGraphDefinitionResolver implements GraphDefinitionResolver
{
    private const PREFIX = 'wireless_';
    ...
    public function definitionFor(string $graphType): GraphDefinition
    {
        ...
        return new WirelessSensorGraph($sensorClass);
    }

    private function sensorClass(string $graphType): ?WirelessSensorType
    {
        if (! str_starts_with($graphType, self::PREFIX)) {
            return null;
        }

        return WirelessSensorType::tryFrom(substr($graphType, strlen(self::PREFIX)));
    }
}
