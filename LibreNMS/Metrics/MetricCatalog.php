...
namespace LibreNMS\Metrics;

use LibreNMS\Data\Store\VictoriaMetrics\MetricCatalogEntry;
use LibreNMS\Data\Store\VictoriaMetrics\MetricDefinition;

/**
 * Backend-neutral metric schema and single source of truth ("decoder ring").
 *
 * Each entry fully describes one metric across backends: the datastore write tuple
 * (measurement + field), the RRD data source used to read it, the VictoriaMetrics
 * metric name/type/unit, the identity labels used to query it, and any descriptive
 * labels written alongside it. The writer (datastore label set) and the readers
 * (RRD ds + VM selector) all derive from this one place, which removes the previous
 * drift between the hand-maintained write allowlist and the per-metric read labels.
 */
class MetricCatalog
{
    /**
     * Descriptive labels that are written for humans/Grafana but are intentionally
     * NOT used as query matchers (they are not part of any metric's identity).
     */
    private const DESCRIPTIVE_WRITE_LABELS = ['ifName', 'module', 'af'];
    ...
    /** @var string[]|null */
    private static ?array $writeLabels = null;
    ...
    /**
     * The full set of tag keys the datastore writer may emit as labels, derived from
     * the union of every metric's identity labels plus the declared descriptive labels.
     * 'hostname' is omitted because it is always written explicitly by the writer.
     *
     * @return string[]
     */
    public static function writeLabels(): array
    {
        if (self::$writeLabels !== null) {
            return self::$writeLabels;
        }

        $labels = [];
        foreach (self::byKey() as $entry) {
            foreach ($entry->identityLabels as $label) {
                $labels[$label] = true;
            }
            foreach ($entry->descriptiveLabels as $label) {
                $labels[$label] = true;
            }
        }
        foreach (self::DESCRIPTIVE_WRITE_LABELS as $label) {
            $labels[$label] = true;
        }
        unset($labels['hostname']);

        return self::$writeLabels = array_keys($labels);
    }
    ...
    private static function buildByKey(): array
    {
        $entries = [
            ...
            self::entry('port.if_in_bits_rate', 'ports', 'ifInBits_rate', 'librenms_port_if_in_bits_per_second', 'gauge', 'bits_per_second', self::portLabels(), derived: ['op' => 'scale', 'inputs' => ['port.if_in_octets'], 'factor' => 8]),
            self::entry('port.if_out_bits_rate', 'ports', 'ifOutBits_rate', 'librenms_port_if_out_bits_per_second', 'gauge', 'bits_per_second', self::portLabels(), derived: ['op' => 'scale', 'inputs' => ['port.if_out_octets'], 'factor' => 8]),
            self::entry('port.if_in_ucast_pkts_rate', 'ports', 'ifInUcastPkts_rate', 'librenms_port_if_in_ucast_pkts_per_second', 'gauge', 'packets_per_second', self::portLabels(), derived: ['op' => 'rate', 'inputs' => ['port.if_in_ucast_pkts']]),
            self::entry('port.if_out_ucast_pkts_rate', 'ports', 'ifOutUcastPkts_rate', 'librenms_port_if_out_ucast_pkts_per_second', 'gauge', 'packets_per_second', self::portLabels(), derived: ['op' => 'rate', 'inputs' => ['port.if_out_ucast_pkts']]),
            self::entry('port.if_in_errors_rate', 'ports', 'ifInErrors_rate', 'librenms_port_if_in_errors_per_second', 'gauge', 'errors_per_second', self::portLabels(), derived: ['op' => 'rate', 'inputs' => ['port.if_in_errors']]),
            self::entry('port.if_out_errors_rate', 'ports', 'ifOutErrors_rate', 'librenms_port_if_out_errors_per_second', 'gauge', 'errors_per_second', self::portLabels(), derived: ['op' => 'rate', 'inputs' => ['port.if_out_errors']]),
