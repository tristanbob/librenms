...
export function mountEChartsGraphs() {
    ...
    containers.forEach((el) => {
        ...
        function graphDataUrl() {
            ...
            parsed.searchParams.set('width', String(graphWidth()));
            parsed.searchParams.set('height', String(graphHeight()));
        ...
        function attachLinkedGraphBehavior() {
            ...
            // In thumbnail rows promote the target to the <td> so the label is included
            const cell   = el.closest('.lnms-echart-thumbnail-cell');
            const target = cell ?? el;

            target.classList.add('lnms-echart-clickable');
